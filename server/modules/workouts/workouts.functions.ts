...
export function computeWorkoutStats(workout: Workout): WorkoutCompute {
  ...
  for (const exercise of workout.exercises) {
    const exerciseVolume = exercise.sets.reduce((sum, s) => sum + s.reps * s.weight, 0);
...
export function compareWorkouts(current: WorkoutCompute, previous: WorkoutCompute): WorkoutCompute {
  const stats: WorkoutCompute = {
    totalVolume: ((current.totalVolume - previous.totalVolume) / previous.totalVolume) * 100,
    maxWeight: ((current.maxWeight - previous.maxWeight) / previous.maxWeight) * 100,
    perExercise: []
  };
...
export function getWorkoutInsights(comparison: WorkoutCompute) {
  ...
  if (comparison.totalVolume > 5)
    improvements.push(`Overall workout volume increased by ${comparison.totalVolume.toFixed(1)}%.`);
  else if (comparison.totalVolume < -5)
    regressions.push(`Overall volume decreased by ${Math.abs(comparison.totalVolume).toFixed(1)}%.`);

  for (const ex of comparison.perExercise) {
    if (ex.volume > 5)
      improvements.push(`${ex.name}: +${ex.volume.toFixed(1)}% volume`);
    else if (ex.volume < -5)
      regressions.push(`${ex.name}: -${Math.abs(ex.volume).toFixed(1)}% volume`);

    if (ex.max > 0)
      highlights.push(`New PR on ${ex.name}: +${ex.max.toFixed(1)}% max weight`);
  }

  if (regressions.length > 2)
    suggestions.push("Consider taking a rest day or lowering intensity.");
  if (improvements.length > 3)
    suggestions.push("Great consistency! Try increasing weights slightly next session.");
